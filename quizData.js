const quizData = [
  // Part 1: Sentence Comprehension (1-15)
  {
    "id": 1,
    "part": 1,
    "question": "那杯牛奶被喝了一半。",
    "image": "images/1.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 2,
    "part": 1,
    "question": "我家一共三口人。",
    "image": "images/2.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 3,
    "part": 1,
    "question": "樹下一個禮物也沒有。",
    "image": "images/3.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 4,
    "part": 1,
    "question": "餐廳的樓上有洗手間。",
    "image": "images/4.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 5,
    "part": 1,
    "question": "沒戴眼鏡的那位先生就是林明華。",
    "image": "images/5.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 6,
    "part": 1,
    "question": "公車快兩點才來。",
    "image": "images/6.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 7,
    "part": 1,
    "question": "學生看著黑板上的句子說話。",
    "image": "images/7.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 8,
    "part": 1,
    "question": "弟弟喜歡用電腦玩遊戲。",
    "image": "images/8.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 9,
    "part": 1,
    "question": "樓梯上的門不知道被誰打開了。",
    "image": "images/9.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 10,
    "part": 1,
    "question": "大明受傷了，他現在躺在醫院，不能站也不能坐。",
    "image": "images/10.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 11,
    "part": 1,
    "question": "哥哥在冰箱旁邊放了一部洗衣機。",
    "image": "images/11.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 12,
    "part": 1,
    "question": "弟弟不會寫書法。",
    "image": "images/12.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 13,
    "part": 1,
    "question": "因為他都把菜吃光了，所以已經吃不下了。",
    "image": "images/13.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 14,
    "part": 1,
    "question": "小文喜歡一邊寫作業，一邊吃東西。",
    "image": "images/14.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 15,
    "part": 1,
    "question": "小華只有右手受傷，其他地方都沒事。",
    "image": "images/15.png",
    "options": ["(A)", "(B)", "(C)"],
    "answer": "B",
    "remark": ""
  },

  // Part 2: Picture Description (16-30)
  {
    "id": 16,
    "part": 2,
    "question": "",
    "image": "images/16.png",
    "options": ["這裡是機場。", "火車到車站了。", "很多人在等車。"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 17,
    "part": 2,
    "question": "",
    "image": "images/17.png",
    "options": ["他們換了新家具。", "他們忘了打開窗戶。", "他們兩個人搬不了沙發。"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 18,
    "part": 2,
    "question": "",
    "image": "images/18.png",
    "options": ["老師準備上課。", "學生們正在休息。", "教室裡有人在說話。"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 19,
    "part": 2,
    "question": "",
    "image": "images/19.png",
    "options": ["郵局在銀行旁邊。", "銀行的對面是郵局。", "車站在郵局的前面。"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 20,
    "part": 2,
    "question": "",
    "image": "images/20.png",
    "options": ["他們點了麵和湯。", "他們覺得很傷心。", "女生坐在男生的右邊。"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 21,
    "part": 2,
    "question": "",
    "image": "images/21.png",
    "options": ["他看不懂這本書。", "他覺得這本書很有趣。", "他看書的時候很緊張。"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 22,
    "part": 2,
    "question": "",
    "image": "images/22.png",
    "options": ["這家飯館中午不休息。", "這家飯館最貴的是雞肉飯。", "這家飯館十一點以後休息。"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 23,
    "part": 2,
    "question": "",
    "image": "images/23.png",
    "options": ["這家店也賣果汁。", "這位小姐買了香蕉牛奶。", "香蕉牛奶便宜了十塊錢。"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 24,
    "part": 2,
    "question": "",
    "image": "images/24.png",
    "options": ["妹妹是十點多離開家的。", "妹妹到家的時候快十點了。", "妹妹十點的時候就要回家了。"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 25,
    "part": 2,
    "question": "",
    "image": "images/25.png",
    "options": ["初級中文課每一次上三個小時。", "初級中文課在二O一教室上課。", "這個班的學生五月三日不上課。"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 26,
    "part": 2,
    "question": "",
    "image": "images/26.png",
    "options": ["病人正在讓醫生檢查。", "醫生要病人試著站起來。", "護士幫忙看病人哪裡受傷了。"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 27,
    "part": 2,
    "question": "",
    "image": "images/27.png",
    "options": ["小華一星期運動四天。", "小華只有星期二下午能休息。", "小華每星期上兩天的中文課。"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 28,
    "part": 2,
    "question": "",
    "image": "images/28.png",
    "options": ["這班公車從火車站出發。", "下午五點還有車到博物館。", "66號公車每兩個鐘頭一班。"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 29,
    "part": 2,
    "question": "",
    "image": "images/29.png",
    "options": ["這間公寓在學校附近。", "公寓裡面只有一個房間。", "想租這間公寓的人上網找陳先生。"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 30,
    "part": 2,
    "question": "",
    "image": "images/30.png",
    "options": ["這個晚會的飲料有三種。", "第二次喝飲料不能用新的杯子。", "每人只能在晚會裡喝一杯飲料。"],
    "answer": "B",
    "remark": ""
  },

  // Part 3: Gap Filling (31-40)
  {
    "id": 31,
    "part": 3,
    "question": "王小姐 ______ 旅遊很有興趣。",
    "image": "",
    "options": ["跟", "和", "對"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 32,
    "part": 3,
    "question": "她放假的時候 ______ 出國玩。",
    "image": "",
    "options": ["多少", "可能", "常常"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 33,
    "part": 3,
    "question": "她跟我說她第一次旅行時，在旅館 ______ 丟了行李。",
    "image": "",
    "options": ["做", "弄", "走"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 34,
    "part": 3,
    "question": "她緊張 ______ 好幾天，三天以後才找到行李。",
    "image": "",
    "options": ["著", "了", "得"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 35,
    "part": 3,
    "question": "從那一次開始，她旅行的時候就更 ______ 自己的東西了。",
    "image": "",
    "options": ["小心", "知道", "認識"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 36,
    "part": 3,
    "question": "我很喜歡到附近一家很大的 ______ 買菜。",
    "image": "",
    "options": ["藥房", "書店", "超市"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 37,
    "part": 3,
    "question": "那裡的東西不怎麼便宜，______比較新鮮，吃起來很放心。",
    "image": "",
    "options": ["但是", "或是", "還是"],
    "answer": "A",
    "remark": ""
  },
  {
    "id": 38,
    "part": 3,
    "question": "聽說今天八點那裡有特別活動，所以我早上六點 ______ 起床了。",
    "image": "",
    "options": ["再", "才", "就"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 39,
    "part": 3,
    "question": "到了那裡以後，我買了好多 ______ 我愛吃的餅乾和一些青菜。",
    "image": "",
    "options": ["枝", "包", "口"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 40,
    "part": 3,
    "question": "不過 ______ 付錢的人太多了，等的時間比買東西的時間還久。",
    "image": "",
    "options": ["掛號", "點菜", "排隊"],
    "answer": "C",
    "remark": ""
  },

  // Part 4: Paragraph Completion (41-45)
  {
    "id": 41,
    "part": 4,
    "question": "",
    "image": "",
    "options": ["買了一間房子", "就有一家大醫院", "林一正沒花多少錢", "林一正花了不少錢", "那裡的生活很方便", "可以常常帶孩子到公園玩"],
    "answer": "A",
    "remark": "Paragraph 1"
  },
  {
    "id": 42,
    "part": 4,
    "question": "",
    "image": "",
    "options": ["買了一間房子", "就有一家大醫院", "林一正沒花多少錢", "林一正花了不少錢", "那裡的生活很方便", "可以常常帶孩子到公園玩"],
    "answer": "B",
    "remark": "Paragraph 1"
  },
  {
    "id": 43,
    "part": 4,
    "question": "",
    "image": "",
    "options": ["買了一間房子", "就有一家大醫院", "林一正沒花多少錢", "林一正花了不少錢", "那裡的生活很方便", "可以常常帶孩子到公園玩"],
    "answer": "F",
    "remark": "Paragraph 1"
  },
  {
    "id": 44,
    "part": 4,
    "question": "",
    "image": "",
    "options": ["買了一間房子", "就有一家大醫院", "林一正沒花多少錢", "林一正花了不少錢", "那裡的生活很方便", "可以常常帶孩子到公園玩"],
    "answer": "D",
    "remark": "Paragraph 1"
  },
  {
    "id": 45,
    "part": 4,
    "question": "",
    "image": "",
    "options": ["買了一間房子", "就有一家大醫院", "林一正沒花多少錢", "林一正花了不少錢", "那裡的生活很方便", "可以常常帶孩子到公園玩"],
    "answer": "E",
    "remark": "Paragraph 1"
  },

  // Part 5: Reading Comprehension (46-50)
  {
    "id": 46,
    "part": 5,
    "question": "明麗的朋友都覺得明麗總是開心地過生活，大家從來沒有看過她傷心或生氣。有一次，我問她是怎麼辦到的，她回答我：「每天一早醒來，我都有兩個選擇，可以選擇壞心情，討厭自己的生活；也可以選擇好心情，喜愛自己的生活。如果是你，不會選第二個嗎？」<br/>Q: 明麗最後要說的意思是什麼？",
    "image": "",
    "options": ["想太多事情對自己沒用", "心情好壞可以自己決定", "早睡早起的人一定快樂", "生活進步的方法有很多"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 47,
    "part": 5,
    "question": "你怎麼處理那些已經不再看的書呢？直接丟掉？送給別人？還是賣給書店？現代網路的進步，使賣書變得更容易。於是有些人就利用網路開店賣舊書，把賺來的錢分給需要的人，有時也將舊書送到鄉下或山裡的學校，這樣的話，不但可以保護環境，還可以讓更多的人有機會讀書。<br/>Q: 這篇文章在談什麼？",
    "image": "",
    "options": ["用家中舊書換新書的方法", "上網看書才不會用太多紙", "學會賣書可以賺更多的錢", "把不要的書變成有用的書"],
    "answer": "D",
    "remark": ""
  },
  {
    "id": 48,
    "part": 5,
    "question": "順文歌唱得很好，上個月，她參加了一個唱歌比賽，為了能把歌唱好，她每天都練習五個小時，非常辛苦。沒想到，就在比賽開始的前一天，順文不小心感冒了，大家都非常擔心這會對她有不好的影響。結果，比賽的成績真的不太好。但順文卻一點也不難過，她告訴我們，能參加比賽就已經很幸福了，比賽讓她學到很多。<br/>Q: 從參加比賽到比賽結束，順文是怎麼想的？",
    "image": "",
    "options": ["她只想拿第一名", "這是一次很好的經驗", "成績不好都是因為生病", "為了比賽，健康已經不重要"],
    "answer": "B",
    "remark": ""
  },
  {
    "id": 49,
    "part": 5,
    "question": "一天工作八小時，有的人覺得剛剛好，有的人覺得不夠用，會出現這樣的不同，我覺得是因為沒有好好安排時間的關係。下面兩點建議可以給大家參考：第一，試試看申請兩個手機號碼，一個只用在工作上，另一個用來聯繫家人或朋友，上班時間只接工作用的電話。第二，每天進辦公室或下班以前，可以留半小時到一小時，收、寄電子郵件，這樣的話，就不會因為經常得花時間寫郵件，影響了自己的工作。<br/>Q: 作者覺得，為什麼有人會覺得上班時間不夠？",
    "image": "",
    "options": ["他們太喜歡找家人、朋友聊天兒", "他們常常忘了回重要的電子郵件", "他們可能經常工作到一半就得停下來", "他們不太常注意自己上、下班的時間"],
    "answer": "C",
    "remark": ""
  },
  {
    "id": 50,
    "part": 5,
    "question": "上週，校長邀請了一位大公司的老闆到學校來，跟同學們談一談他的成功經驗，講完以後，還讓同學們問問題。因為那位老闆講得太好，同學們的問題一個接著一個，大家最想知道的就是：要怎麼做才能快一點兒成功？我本來以為，大老闆會教大家一些實際的方法，但他卻告訴我們：「做好現在應該做的，你們就已經成功了」。<br/>Q: 老闆聽了同學們最想問的問題以後，他怎麼回答？",
    "image": "",
    "options": ["好好學習就可以了", "必須像他一樣努力才行", "要知道什麼事情不應該做", "得學會馬上聽懂老闆說的話"],
    "answer": "A",
    "remark": ""
  }
];
